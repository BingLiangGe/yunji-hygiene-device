...
public class RedisCache {
    ...
    public void deleteByPattern(String pattern) {
        ...
        try (Cursor<byte[]> cursor = connection.scan(
                new ScanOptions.ScanOptionsBuilder().match(pattern).count(1000).build()
        )) {
            List<byte[]> batch = Lists.newArrayList();
            cursor.forEachRemaining(key -> {
                batch.add(key);
                if (batch.size() >= 1000)
                    pipelineRemove(connection, batch);
            });
            if (!batch.isEmpty())
                pipelineRemove(connection, batch);
        }
    ...
    private static void pipelineRemove(RedisConnection connection, List<byte[]> batch) {
    ...
    public List<String> scanPattern(String pattern) {
        RedisConnection connection = redisTemplate.getRequiredConnectionFactory().getConnection();
